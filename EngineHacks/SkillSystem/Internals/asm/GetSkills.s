...
no_personal:
	...
	@new function: if unit has Canto as class skill, they don't get their personal utility skill
	cmp r2, #CantoID
	bne StoreClassSkill
...
lop_end:
	@if unit is enemy or NPC, go to generic_unit
	ldrb r0, [r4,#0x0B]
	lsr r0, #6
	cmp r0, #0
	bne generic_unit
...
generic_unit:
	...
	@ new function: storing a skill in the byte for fatigue for generics when they're loaded in by events
	...
	mov r1, #0x3B
	ldrb r2, [r0,r1] @ skill byte

	cmp r2, #0
	beq no_stored_generic_skill

	strb r2, [r5]
	add  r5, #1
	
no_stored_generic_skill:
