...
@check if zero hit
  @if so, automatic miss
ldrh    r0, [r7,#0xA]      @final hit rate
cmp		r0, #0
beq		Missed

@check if Perfectus is active
  @if so, automatic hit
mov		r0, r4
ldr		r3, =IsPerfectusActive
mov		lr, r3
.short 	0xF800

cmp		r0, #0
bne		SuccessfulHitFullDamage

@check if other unit has active Perfectus
  @if so, automatic miss
mov		r0, r5
ldr		r3, =IsPerfectusActive
mov		lr, r3
...
@if enemy, skip hit roll
ldrb	r0, [r4,#0x0B]
cmp		r0, #0x80
bgt		SuccessfulHitFullDamage
...
bne     SuccessfulHitFullDamage        @If we hit, branch                @ 0802B426 D111   

@hit roll: attempt 2
ldrh    r0,[r7,#0xA]      @final hit rate                @ 0802B41A 8960
@sub		r0, #0x0A
@cmp		r0, #0x00 @compares final hit to 0; if less than or equal, no reroll
@ble		Missed
...
@mov		r1,#0x62
@ldrh	r1,[r5,r1]
@sub		r0,r1 @reroll chance of hitting = final hit*2 - enemy avoid
@cmp		r0, #0x00 @compares final hit to 0; if less than or equal, no reroll
@ble		Missed
@mov     r1,#0x1           @Default depending on where battle is called, leave it alone             @ 0802B41C 2101     
@blh     proc_truehit        @Proc hit rate                @ 0802B41E F7FFF89B 
...
mov 	r1, #0
blh 	d100Result @chance of glancing blow is 1RN instead of 2RN
mov		r2, #1  
cmp     r0, #0                @ 0802B424 2800     
bne     SuccessfulHitHalfDamage        @If we hit, branch                @ 0802B426 D111   
...
SuccessfulHitFullDamage:
mov r2, #0

SuccessfulHitHalfDamage:
...
add		r0, #0x4A
ldrh	r0, [r0]
ldr 	r3, =#0x08017724 @get weapon effect
mov		lr, r3
.short	0xF800
...
cmp r0, #0 @if final crit is 0, just skip to end
beq End

@check if Perfectus is active
  @if so, automatic success
mov		r0, r4
ldr		r3, =IsPerfectusActive
mov		lr, r3
.short 	0xF800
...
@check for killer weapon; if not on table, skip to End
ldr r1, =KillerWeaponTable
ldrb r2, [r4,#0x1E]

LoopThroughKillerWeaponTable:
ldrb r0, [r1]
add r1, #1
cmp r0, #0
beq End
...
cmp r0, r2
bne LoopThroughKillerWeaponTable

@check if Prime Five is active; if so, use Lck instead of Skl from wielder
mov		r0, r4
...
@check if Skl > enemy Skl by at least 4
CheckEnemySkl:
ldrb	r2, [r5,#0x15]
sub		r1, r2
cmp		r1, #4
...
mov 	r1, #4
ldrsh 	r1, [r7, r1] @damage
ldrh 	r2, [r7, #0x0C] @crit rate
cmp		r2, #9
ble		CritRange1 @1%-9%
cmp		r2, #19
ble		CritRange2 @10%-19%
cmp		r2, #29
ble		CritRange3 @20%-29%

@CritRange4, which is 30%+
lsl		r2, r1, #0x01
b		ApplyExpertiseIfApplicable
